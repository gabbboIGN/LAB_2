...
function eliminar (req, res){
    let idpersona = req.params.id 
    Persona.findById(idpersona,(err,persona)=>{ 
         if (err) restart.status(500).send ({message:'Error al borrar el Persona:'})
         persona.remove (err =>{
...
function put(req ,res ){
    ...
    Persona.findByIdAndUpdate(idpersona , update , (err ,personaUpdate )=>{
       if (err) res.status(500).send ({message: 'Error al actualizar Persona'})
